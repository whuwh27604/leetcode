/* 到达最后一个房间的最少时间 II.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
...
public:
    ...
    Node() : time(0), moves(1), r(0), c(0) {}
    Node(int t, int m, int x, int y) : time(t), moves(m), r(x), c(y) {}
        ...
        while (!nodes.empty()) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (nr >= 0 && nr < row && nc >= 0 && nc < column) {
                    int time = max(node.time, moveTime[nr][nc]) + node.moves;
                    if (time < minTimes[nr][nc]) {
                        minTimes[nr][nc] = time;
                        nodes.push(Node(time, (node.moves ^ 3), nr, nc));
                    }
                }
            }
        }
...
int main()
{
    ...
    check.checkInt(7, o.minTimeToReach(moveTime));

    moveTime = { {0,0,0,0},{0,0,0,0} };
    check.checkInt(6, o.minTimeToReach(moveTime));

    moveTime = { {0,1},{1,2} };
    check.checkInt(4, o.minTimeToReach(moveTime));
}
