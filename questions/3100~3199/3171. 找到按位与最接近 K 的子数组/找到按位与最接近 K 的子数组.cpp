...
public:
    int minimumDifference(vector<int>& nums, int k) {
        int size = (int)nums.size(), left = 0, right = 0, sum = INT_MAX, minDiff = INT_MAX;
        vector<int> count(maxSize, 0);  // 关键是窗口左端移除时，如何快速计算sum，通过count统计每个bit位来解决这个问题

        for (; left < size; ++left) {
            while (right < size && ((sum & nums[right]) >= k)) {  // sum还大于k时，可以继续扩大窗口
                sum &= nums[right];
                add(count, nums[right++]);
            }

            minDiff = min(minDiff, sum - k);  // 达到临界点，计算结果
            if (right < size) {
                minDiff = min(minDiff, k - (sum & nums[right]));
            }

            if (right > left) {  // 如果有合法窗口，移除最左端
                minus(count, nums[left], right - left - 1, sum);
            }
    ...
    void minus(vector<int>& count, int num, int len, int& sum) {
        for (int i = 0, bit = 1; i < maxSize; ++i, bit <<= 1) {
            count[i] -= (num & 1);
            if (count[i] == len) {  // 移除左端后，剩下的1的个数是窗口长度，说明sum的该bit位为1
                sum |= bit;
            }
            num >>= 1;
        }
...
int main()
{
    ...
    check.checkInt(1, o.minimumDifference(nums, 3));

    nums = { 1,2,1,2 };
    check.checkInt(0, o.minimumDifference(nums, 2));

    nums = { 1 };
    check.checkInt(9, o.minimumDifference(nums, 10));

    nums = { 1,2,4,5,1,1,2,1,2 };
    check.checkInt(3, o.minimumDifference(nums, 8));
}
