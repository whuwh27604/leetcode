...
public:
    ...
    void search(string& s, int start, vector<int>& matchIndices) {
        TrieNode* current = root;
...
public:
    bool wordBreak(string s, vector<string>& wordDict) {
        ...
        // 令dp[j]表示s[j~size-1]是否能够分割，如果dp[j]=true，那么dp[i]=true, if (s[i~j] is a word)
        int i, size = s.size();
