...
public:
    vector<int> missingTwo(vector<int>& nums) {
        int n = nums.size() + 2;
        int xorSum = getXorSum(n, nums);
        int lowbit = (xorSum & -xorSum);

        return getMissingTwo(n, nums, lowbit);
    }

    int getXorSum(int n, vector<int>& nums) {
        int xorSum = 0;
    ...
    vector<int> getMissingTwo(int n, vector<int>& nums, int lowbit) {
