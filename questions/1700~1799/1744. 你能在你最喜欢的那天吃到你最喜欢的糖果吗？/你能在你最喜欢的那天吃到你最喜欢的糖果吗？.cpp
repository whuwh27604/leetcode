/* 你能在你最喜欢的那天吃到你最喜欢的糖果吗？.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
...
public:
    vector<bool> canEat(vector<int>& candiesCount, vector<vector<int>>& queries) {
        ...
        vector<long long> presum(candiesCount.size() + 1);

        getPresum(candiesCount, presum);

        for (i = 0; i < size; ++i) {
            int type = queries[i][0], day = queries[i][1];
            long long minimun = (long long)day + 1, maximum = minimun * queries[i][2];
            ans[i] = !((minimun > presum[type + 1]) || (maximum < presum[type] + 1));
        }

        return ans;
    }

    void getPresum(vector<int>& candiesCount, vector<long long>& presum) {
        presum[0] = 0;

        for (unsigned int i = 0; i < candiesCount.size(); ++i) {
...
int main()
{
    ...
    vector<bool> actual = o.canEat(candiesCount, queries);
    ...
    actual = o.canEat(candiesCount, queries);
