/* 将单词恢复初始状态所需的最短时间 II.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
...
public:
    int minimumTimeToInitialState(string word, int k) {
		int n = (int)word.size(), time = 1;
		vector<int> z = functionZ(word);
...
int main()
{
	...
	check.checkInt(1, o.minimumTimeToInitialState("z", 1));
	check.checkInt(2, o.minimumTimeToInitialState("baba", 3));
}
