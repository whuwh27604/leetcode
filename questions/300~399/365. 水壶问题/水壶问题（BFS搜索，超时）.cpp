...
public:
    bool canMeasureWater(int x, int y, int z) {
        unordered_set<unsigned long long> remainXY;
        stack <pair<int, int>> remains;
        remains.push({ 0,0 });
        int remainX = 0, remainY = 0;

        while (!remains.empty()) {
            int remainX = remains.top().first;
            int remainY = remains.top().second;
            remains.pop();

            if (((remainX + remainY) == z) || (remainX == z) || (remainY == z)) {
                return true;
            }

            if (remainXY.find(((unsigned long long)remainX << 32) + (unsigned long long)remainY) != remainXY.end()) {
                continue;
            }

            remainXY.insert(((unsigned long long)remainX << 32) + (unsigned long long)remainY);

            // 装满任意一个水壶
            remains.push({ x, remainY });
            remains.push({ remainX,y });

            // 清空任意一个水壶
            remains.push({ 0, remainY });
            remains.push({ remainX,0 });

            // 从一个水壶向另外一个水壶倒水，直到装满或者倒空
            int tmpX = (remainX > (y - remainY)) ? (remainX + remainY - y) : 0;  // x->y
            int tmpY = remainX + remainY - tmpX;
            remains.push({ tmpX, tmpY });
            tmpX = (remainY > (x - remainX)) ? x : (remainX + remainY);  // y->x
            tmpY = remainX + remainY - tmpX;
            remains.push({ tmpX, tmpY });
        }
