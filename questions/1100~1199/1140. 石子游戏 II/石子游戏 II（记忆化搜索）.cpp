...
public:
    int stoneGameII(vector<int>& piles) {
        ...
        return DFS(piles, preSum, memo, 0, 1);
    }
    ...
    int DFS(vector<int>& piles, vector<int>& preSum, unordered_map<int, int>& memo, int start, int M) {
        int i, size = piles.size(), maxStones = 0;
        ...
        for (i = start; i < (start + 2 * M) && i < size; ++i) {
            int stones = preSum[i + 1] - preSum[start];  // 取[start,i]的stone
            int newM = max(M, i - start + 1);
            // 自己取[start,i]以后，还剩preSum[size] - preSum[i+1]，对手最多取DFS()，剩下的是自己的
            stones += (preSum[size] - preSum[i + 1] - DFS(piles, preSum, memo, i + 1, newM));
