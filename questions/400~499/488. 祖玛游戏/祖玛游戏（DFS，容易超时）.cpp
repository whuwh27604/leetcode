...
public:
    int findMinStep(string board, string hand) {
        ...
        int minStep = findMinStep(board, hand, visited);

        return minStep >= inf ? -1 : minStep;
    }

    int findMinStep(string board, string& hand, unordered_map<string, int>& visited) {
        string key = board + "*" + hand;
        auto iter = visited.find(key);
        if (iter != visited.end()) {
            return iter->second;
        }

        string removed = remove(board);
        if (removed.empty()) {
            visited[key] = 0;
            return 0;
        }
        ...
        for (char& color : hand) {
            ...
            minStep = min(minStep, findMinStep(string(1, prevColor) + removed, hand, visited) + 1);

            for (int i = 0; i < (int)removed.size(); ++i) {
                if (prevColor == removed[i]) {
                    continue;  // 相同颜色的球，只插入到开头的位置
                }

                string newBoard(removed, 0, i + 1);
                newBoard += prevColor;
                newBoard += string(removed, i + 1, (int)removed.size() - i - 1);
                minStep = min(minStep, findMinStep(newBoard, hand, visited) + 1);
            }
