/* 单词的压缩编码.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
...
public:
    int minimumLengthEncoding(vector<string>& words) {
        ...
        int totalLen = 0;
        countLength(root, 0, totalLen);

        return totalLen;
    }

    void buildDict(TrieNode* root, vector<string> words) {
    ...
    void countLength(TrieNode* root, int len, int& totalLen) {
        bool isLeaf = true;

        for (int i = 0; i < 26; i++) {
            if (root->next[i] != NULL) {
                isLeaf = false;
                countLength(root->next[i], len + 1, totalLen);
            }            
        }
...
int main()
{
    ...
    check.checkInt(10, o.minimumLengthEncoding(words));

    words = { "a" };
    check.checkInt(2, o.minimumLengthEncoding(words));

    words = { "a","b","c" };
    check.checkInt(6, o.minimumLengthEncoding(words));

    words = { "a","ba","cba" };
    check.checkInt(4, o.minimumLengthEncoding(words));

    words = { "a","ba","cba","dba" };
    check.checkInt(8, o.minimumLengthEncoding(words));

    words = { "blfis","blfisk" };
    check.checkInt(13, o.minimumLengthEncoding(words));

    words = { "atime", "time", "btime", "bbtime" };
    check.checkInt(13, o.minimumLengthEncoding(words));
    ...
    check.checkInt(13956, o.minimumLengthEncoding(words));
}
